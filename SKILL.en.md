...
Prefer running the package `init.js` first so it copies out the real native `et` binary, then point MCP directly at that file.
...
If the host application cannot resolve commands from PATH, the legacy path-based approach still works:
...
If you do not want to run `init.js`, you can also launch through `npx`. The first run can be slower because it may install the current platform package automatically:
...
On Linux / macOS, switch `command` back to `npx` for this fallback configuration.

If a Windows host reports `LOCAL_PROCESS_ERROR`, switch to the native `et.exe` generated by `init.js` first; if you still rely on npm bridge commands, then check whether this field is still set to plain `npx`.
